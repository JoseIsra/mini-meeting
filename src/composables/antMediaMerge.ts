...
const webRTCInstance2 = ref<WebRTCAdaptor>({} as WebRTCAdaptor);
...
export function useInitMerge() {
  ...
  const mergeStreams = (streamId: string) => {
    ...
    setTimeout(() => {
      ...
      console.log(result);
      webRTCInstance2.value.gotStream(result);
  ...
  const joinRoom = (roomId: string, streamId: string) => {
    webRTCInstance2.value.joinRoom?.(roomId, streamId, 'legacy');
  };
  ...
  const leaveRoom = (roomId: string) => {
    webRTCInstance2.value.leaveFromRoom?.(roomId);
  };
  ...
  const createMergeInstance = (
    ...
    webRTCInstance2.value = new WebRTCAdaptor({
      ...
      callback: (info: string, obj: objWebRTC) => {
        if (info == 'initialized') {
          ...
          if (!userMe.isCameraOn) {
            webRTCInstance2.value.turnOffLocalCamera?.(streamId);
          }
          ...
          roomTimerId.value = setInterval(() => {
            webRTCInstance2.value.getRoomInfo?.(roomId, streamId);
          }, 2000);
          ...
          webRTCInstance2.value.resetDesktop?.();

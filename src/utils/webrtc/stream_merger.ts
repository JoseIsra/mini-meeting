...
export class StreamMerger implements StreamMergerType {
  ...
  addStream(mediaStream, options) {
    ...
    options.x == undefined
      ? (stream.x = stream.width * stream.Xindex)
      : (stream.x = options.x);
    options.x == undefined
      ? (stream.y = stream.height * stream.Yindex)
      : (stream.y = options.y);
    ...
    if (!stream.mute) {
      ...
      stream.audioSource
        .connect(stream.audioGainNode)
        .connect(this.audioDestination); // Default is direct connect
    }
    ...
    if (this.autoMode == true) {
      ...
      videoElement.onloadedmetadata = () => {
        ...
        if (pheight > pwidth) {
          ...
          console.log(
            'Location offset from metadata x = ' + stream.x + ' y = ' + stream.y
          );
        }
      };
    }
  ...
  resizeAndSortV2() {
    ...
    for (let i = 1; i <= this.streams.length; i++) {
      ...
      if (extraStreams <= 0 || this.streams.length <= 3) {
        ...
        console.log(
          'Video width = ' + stream.width + 'Video height = ' + stream.height
        );
        ...
        if (yindex >= yNumber - 1 && this.streams.length != 1) {
          console.log(
            'TopWidth = ' + topWidth + ' remainingStreams = ' + remainingStreams
          );
        ...
        console.log(
          'Video Width = ' + stream.width + 'VideoHeight = ' + stream.height
        );
